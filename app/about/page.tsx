...
import { ArrowRight, CodeXml, ExternalLink, UserRound } from "lucide-react";
import { Container } from "@/components/ui/container";
import { PageHero } from "@/components/ui/page-hero";
import { safePublicUrl } from "@/lib/public-profile";

export const metadata: Metadata = { title: "About", description: "About the independent student research project behind Mid-Atlantic CRE Intelligence.", alternates: { canonical: "/about" } };
...
export default function AboutPage() {
  ...
  const founderName = process.env.NEXT_PUBLIC_FOUNDER_NAME?.trim() || "Student researcher and builder";
  const founderRole = process.env.NEXT_PUBLIC_FOUNDER_ROLE?.trim() || "Research, data, and product development";
  const linkedinUrl = safePublicUrl(process.env.NEXT_PUBLIC_LINKEDIN_URL);
  const githubUrl = safePublicUrl(process.env.NEXT_PUBLIC_GITHUB_URL);
  const resumeUrl = safePublicUrl(process.env.NEXT_PUBLIC_RESUME_URL);
  ...
  return <>
    ...
    <Container className="py-16 lg:py-24">
      <section className="grid gap-8 border-b border-line pb-16 lg:grid-cols-[0.62fr_1fr] lg:gap-16" aria-labelledby="founder-title">
        <div className="overflow-hidden bg-navy text-white shadow-[0_20px_50px_rgba(11,34,57,0.16)]"><div className="grid min-h-64 place-items-center border-b border-white/15 bg-ink"><div className="grid size-28 place-items-center border border-white/20"><UserRound aria-hidden="true" className="size-10 text-copper" strokeWidth={1.2} /></div></div><div className="p-7"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-copper">Founder profile</p><h2 className="mt-4 font-serif text-3xl font-semibold" id="founder-title">{founderName}</h2><p className="mt-2 text-sm text-white/68">{founderRole}</p><div className="mt-7 flex flex-wrap gap-3">{linkedinUrl ? <a className="inline-flex min-h-10 items-center gap-2 border border-white/20 px-3 text-xs font-semibold hover:border-white/50" href={linkedinUrl} rel="noreferrer" target="_blank"><ExternalLink aria-hidden="true" className="size-4" /> LinkedIn</a> : null}{githubUrl ? <a className="inline-flex min-h-10 items-center gap-2 border border-white/20 px-3 text-xs font-semibold hover:border-white/50" href={githubUrl} rel="noreferrer" target="_blank"><CodeXml aria-hidden="true" className="size-4" /> GitHub</a> : null}{resumeUrl ? <a className="inline-flex min-h-10 items-center border border-white/20 px-3 text-xs font-semibold hover:border-white/50" href={resumeUrl} rel="noreferrer" target="_blank">Resume</a> : null}</div></div></div>
        <div><p className="eyebrow">Why it exists</p><h2 className="mt-4 max-w-3xl font-serif text-4xl font-semibold leading-tight text-navy">Commercial real estate information is fragmented. The product makes the research trail inspectable.</h2><p className="mt-6 max-w-3xl text-base leading-8 text-slate">Public records, company filings, local reporting, and transaction announcements often describe different parts of the same event. Mid-Atlantic CRE Intelligence explores how consistent definitions, transparent calculations, and visible source records can make that evidence easier to evaluate.</p><p className="mt-5 max-w-3xl text-base leading-8 text-slate">The project also serves as a working demonstration of full-stack product judgment: deciding what to collect, what not to infer, how to structure it, and how to communicate uncertainty without sacrificing usability.</p><div className="mt-8 flex flex-wrap gap-3"><Link className="button-primary gap-2" href="/project">Read the case study <ArrowRight aria-hidden="true" className="size-4" /></Link><Link className="button-secondary" href="/methodology">Review methodology</Link></div></div>
      </section>
