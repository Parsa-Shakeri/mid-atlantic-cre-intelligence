...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const corePages: MetadataRoute.Sitemap = ["", "/comparables", "/properties", "/research", "/dashboard", "/coverage", "/project", "/changelog", "/corrections", "/about", "/methodology"].map((path) => ({ url: `${baseUrl}${path}`, lastModified: new Date(), changeFrequency: path === "" || path === "/comparables" || path === "/properties" || path === "/research" || path === "/dashboard" || path === "/coverage" || path === "/changelog" ? "weekly" : "monthly", priority: path === "" ? 1 : path === "/comparables" ? 0.9 : 0.7 }));
