...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const corePages: MetadataRoute.Sitemap = ["", "/properties", "/research", "/dashboard", "/about", "/methodology"].map((path) => ({ url: `${baseUrl}${path}`, lastModified: new Date(), changeFrequency: path === "" || path === "/properties" || path === "/research" || path === "/dashboard" ? "weekly" : "monthly", priority: path === "" ? 1 : 0.7 }));
