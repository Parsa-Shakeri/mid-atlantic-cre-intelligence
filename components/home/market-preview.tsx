...
export function MarketPreview({ records }: { records: PropertyListItem[] }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[1.45fr_0.75fr]">
    <div className="panel p-6 sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Transaction mix</p><h3 className="mt-3 font-serif text-3xl font-semibold text-navy">Activity by property type</h3></div><span className="tag">n = {records.length}</span></div><div aria-label={`Bar chart of ${records.length} recent transactions by property type`} className="mt-9 grid gap-5" role="img">{labels.map((label, index) => <div className="grid grid-cols-[110px_1fr_28px] items-center gap-3" key={label}><span className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate">{label}</span><div className="h-3 bg-mist"><div className="h-full bg-navy" style={{ width: `${Math.max((counts[index] / max) * 100, 8)}%` }} /></div><span className="text-right font-serif text-lg font-semibold text-navy">{counts[index]}</span></div>)}</div><p className="mt-8 border-t border-line pt-4 text-xs leading-5 text-slate">Counts reflect the same bounded set of recent records displayed above.</p></div>
