...
export function MetricGrid({ summary }: { summary: SummaryMetrics }) {
  ...
  const metrics = [
    [unavailable ? "—" : summary.properties, "Properties tracked"],
    [unavailable ? "—" : summary.transactions, "Transactions analyzed"],
    [unavailable ? "—" : formatCompactCurrency(summary.totalValue), "Transaction value"],
    [unavailable ? "—" : summary.markets, "Markets covered"],
    [unavailable ? "—" : summary.reports, "Published reports"],
  ];
